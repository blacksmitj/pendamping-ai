...
export default function ParticipantDetailClient({
    ...
    const [statusData, setStatusData] = React.useState({
        communicationStatus: p.communicationStatus || "smooth",
        fundDisbursementStatus: p.fundDisbursementStatus || "stage1",
    ...
    const mockParticipant = {
        ...
        status: p.status || "active",
        mentor: p.assignments?.[0]?.mentor?.name || "-",
        university: p.university?.name || "-",
        nik: p.registrantNationalId || "-",
        email: "-", // No email field
        phone: p.whatsapp || "-",
        education: p.lastEducation || "-",
        disability: p.isDisabled ? (p.disabilityType || "Yes") : "No",
        address: p.businessAddress || "-",
        city: p.businessCity || "-",
        province: p.businessProvince || "-",
        baseline: {
            revenue: Number(p.revenuePerPeriod) || 0,
            volume: Number(p.productCountPerPeriod) || 0,
            unit: p.productUnitPerPeriod || "Unit",
            workers: p.partnerCount || 0,
        }
    }

    const mappedLogbooks = p.logbookParticipants?.map((lp: any) => lp.logbook) || []
    const mappedCapaian = p.outputs || []
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <Table>
                                ...
                                <TableBody>
                                    {mappedLogbooks.map((log: any) => (
                                        <TableRow key={log.id} className="hover:bg-slate-50/50">
                                            <TableCell className="font-medium text-slate-900">{new Date(log.date).toLocaleDateString('id-ID')}</TableCell>
                                            <TableCell>
                                            ...
                                            <TableCell>
                                                <Badge variant="secondary" className={log.deliveryMethod === 'FACE_TO_FACE' ? 'bg-indigo-50 text-indigo-700' : 'bg-rose-50 text-rose-700'}>
                        ...
                        <CardContent>
                            <Table>
                                ...
                                <TableBody>
                                    ...
                                    {mappedCapaian.map((cap: any) => (
                                        <TableRow key={cap.id} className="hover:bg-slate-50/50">
                                            <TableCell className="font-bold text-slate-900">{new Date(cap.reportMonth).toLocaleDateString('id-ID', { month: 'long', year: 'numeric' })}</TableCell>
                                            <TableCell className="text-right font-medium text-indigo-600">{new Intl.NumberFormat('id-ID').format(Number(cap.revenue))}</TableCell>
                                            <TableCell className="text-right">{cap.salesVolume} {cap.salesVolumeUnit}</TableCell>
                                            <TableCell className="text-right font-bold">{cap.workerConfirmation}</TableCell>
                                            <TableCell>
                                                <Badge variant={cap.reviewStatus === 'APPROVED' ? 'default' : 'secondary'} className={cap.reviewStatus === 'APPROVED' ? 'bg-emerald-500' : 'bg-amber-500 text-white border-none'}>
                                                    {cap.reviewStatus}
                                                </Badge>
                                            </TableCell>
                                            <TableCell className="text-right">
                                                <Button variant="ghost" size="icon" className="h-8 w-8 text-indigo-600" onClick={() => router.push(`/output/${cap.id}`)}>
                    ...
                    <Card>
                        ...
                        <CardFooter className="bg-slate-50 rounded-b-xl py-4 justify-between">
                            <p className="text-xs text-slate-500 italic flex items-center gap-1.5">
                                <Clock className="h-3 w-3" /> Last updated: {new Date(p.updatedAt).toLocaleDateString('id-ID')}
                            </p>
                            <Button onClick={handleStatusUpdate} disabled={updateStatusMutation.isPending} className="bg-indigo-600 hover:bg-indigo-700 shadow-indigo-100 shadow-lg">
                                <Save className="mr-2 h-4 w-4" /> {updateStatusMutation.isPending ? "Saving..." : "Save Status Update"}
                            </Button>
                        </CardFooter>
                    </Card>
                </TabsContent>
